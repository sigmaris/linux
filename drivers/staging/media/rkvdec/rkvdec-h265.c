...
// SCALING_LIST: 0-15 = sps_id, 16-79 = pps_id, 80 = default

/* Size with u32 units. */
#define RKV_RPS_SIZE			((600 * 32) / 4)
#define RKV_SCALING_LIST_SIZE		((16 + 64 + 1) * 1360)

struct rkvdec_sps_pps_packet {
#if 0 // v345
	u32 info[24]; // 96 bytes
#else
	u32 info[20]; // 80 bytes
#endif
};
...
#define SPS_MAX_DEC_PIC_BUFFERING_MINUS1		PS_FIELD(107, 4)


/* Data structure describing auxiliary buffer format. */
struct rkvdec_h265_priv_tbl {
...
static const u8 rkvdec_h265_cabac_table[27456] = {
...
static void assemble_hw_pps(struct rkvdec_ctx *ctx,
			    struct rkvdec_h265_run *run)
{
	struct rkvdec_dev *rkvdec = ctx->dev;
	struct rkvdec_h265_ctx *h265_ctx = ctx->priv;
	const struct v4l2_ctrl_hevc_slice_params *sl_params = &run->slices_params[0];
	const struct v4l2_ctrl_hevc_sps *sps = run->sps;
	const struct v4l2_ctrl_hevc_pps *pps = run->pps;
	struct rkvdec_h265_priv_tbl *priv_tbl = h265_ctx->priv_tbl.cpu;
	struct rkvdec_sps_pps_packet *hw_ps;
	u32 min_cb_log2_size_y, ctb_log2_size_y, min_cb_size_y, ctb_size_y;
	u32 pic_width_in_min_cbs_y, pic_height_in_min_cbs_y;
	u32 width, height, log2_min_cu_qp_delta_size;
	 ...
	 */
	hw_ps = &priv_tbl->param_set[0];
	//memset(hw_ps, 0, sizeof(*hw_ps));
	memset(hw_ps, 0, 80 * 64);
	...
	pic_height_in_min_cbs_y = sps->pic_height_in_luma_samples /
	    min_cb_size_y;

	width = pic_width_in_min_cbs_y << min_cb_log2_size_y;
	height = pic_height_in_min_cbs_y << min_cb_log2_size_y;

	dev_info(rkvdec->dev, "width=%u height=%u ctb_size_y=%u\n", width, height, ctb_size_y);

#define WRITE_PPS(value, field) set_ps_field(hw_ps->info, field, value)
	/* write sps */
	WRITE_PPS(0, VIDEO_PARAMETER_SET_ID);
	WRITE_PPS(0, SEQ_PARAMETER_SET_ID);
	WRITE_PPS(sps->chroma_format_idc, CHROMA_FORMAT_IDC);
	WRITE_PPS(width, PIC_WIDTH_IN_LUMA_SAMPLES);
	WRITE_PPS(height, PIC_HEIGHT_IN_LUMA_SAMPLES);
	...
	//	  SCALING_LIST_ENABLED_FLAG);
        WRITE_PPS(1, SCALING_LIST_ENABLED_FLAG);
...
#if 0 // v345
	WRITE_PPS(sps->sps_max_dec_pic_buffering_minus1,
...
#define CU_QP_DELTA_ENABLED_FLAG			PS_FIELD(172, 1)
	...
	WRITE_PPS(0, PPS_SEQ_PARAMETER_SET_ID);
	...
	WRITE_PPS(sl_params->num_ref_idx_l0_active_minus1 + 1,
		  NUM_REF_IDX_L0_DEFAULT_ACTIVE);
	WRITE_PPS(sl_params->num_ref_idx_l1_active_minus1 + 1,
...
#define SLICE_SEGMENT_HEADER_EXTENSION_PRESENT_FLAG	PS_FIELD(207, 1)

#define NUM_TILE_COLUMNS				PS_FIELD(211, 5)
#define NUM_TILE_ROWS					PS_FIELD(216, 5)
...
#define SCALING_LIST_ADDRESS				PS_FIELD(592, 32)
	...
	if (pps->flags & V4L2_HEVC_PPS_FLAG_TILES_ENABLED) {
		...
		WRITE_PPS(((width + ctb_size_y - 1) / ctb_size_y) - 1, COLUMN_WIDTH(0));
		WRITE_PPS(((height + ctb_size_y - 1) / ctb_size_y) - 1, ROW_HEIGHT(0));
	}

	// TODO: scaling list
	if (sps->flags & V4L2_HEVC_SPS_FLAG_SCALING_LIST_ENABLED) {
		dev_info(rkvdec->dev, "scaling list enabled\n");
	}

	scaling_distance = offsetof(struct rkvdec_h265_priv_tbl, scaling_list);
	scaling_list_address = h265_ctx->priv_tbl.dma + scaling_distance;
	WRITE_PPS(scaling_list_address, SCALING_LIST_ADDRESS);
	WRITE_PPS(0xffff, PS_FIELD(624, 16));

	for (i = 0; i < 20; i++)
		if (hw_ps->info[i])
			dev_info(rkvdec->dev, "pps[%d]=%08x\n", i, hw_ps->info[i]);

	for (i = 1; i < 64; i++)
		memcpy(&priv_tbl->param_set[i], hw_ps, 80);
}
			    ...
			    struct rkvdec_h265_run *run)
{
	...
	//const struct v4l2_ctrl_h265_sps *sps = run->sps;
	//struct rkvdec_h265_ctx *h265_ctx = ctx->priv;
	//struct rkvdec_h265_priv_tbl *priv_tbl = h265_ctx->priv_tbl.cpu;
        int j;
	...
	//memset(hw_rps, 0, (run->num_slices + 1) * 32);

	for (j = 0; j < run->num_slices; j++) {
		sl_params = &run->slices_params[j];
		dpb = sl_params->dpb;

		dev_info(rkvdec->dev, "slice[%d]: slice_type=%d num_active_dpb_entries=%d num_ref_idx_l0_active_minus1=%d num_ref_idx_l1_active_minus1=%d\n", j,
			 sl_params->slice_type, sl_params->num_active_dpb_entries, sl_params->num_ref_idx_l0_active_minus1, sl_params->num_ref_idx_l1_active_minus1);

	//if (sl_params->slice_type != V4L2_HEVC_SLICE_TYPE_I) {
	//for (i = 0; i <= sl_params->num_ref_idx_l0_active_minus1; i++) {
	//	rbsp_write_flag(rbsp, !!(dpb[sl_params->ref_idx_l0[i]].rps == V4L2_HEVC_DPB_ENTRY_RPS_LT_CURR));
	//	rbsp_write_bits(rbsp, 4, sl_params->ref_idx_l0[i]);
	//}
	//}

	//if (sl_params->slice_type == V4L2_HEVC_SLICE_TYPE_B) {
	//for (i = 0; i <= sl_params->num_ref_idx_l1_active_minus1; i++) {
	//	rbsp_write_flag(rbsp, !!(dpb[sl_params->ref_idx_l1[i]].rps == V4L2_HEVC_DPB_ENTRY_RPS_LT_CURR));
	//	rbsp_write_bits(rbsp, 4, sl_params->ref_idx_l1[i]);
	//}
	//}
				 ...
				 struct rkvdec_h265_run *run)
{
	...
	struct rkvdec_h265_ctx *h265_ctx = ctx->priv;
	struct rkvdec_h265_priv_tbl *tbl = h265_ctx->priv_tbl.cpu;
	u8 *dst;
	scalingList_t sl;
	int i, j;

	memcpy(tbl->cabac_table, rkvdec_h265_cabac_table,
	       sizeof(rkvdec_h265_cabac_table));

	//if (!memcmp((void*)&run->scaling_matrix_cache, scaling,
	...
	hal_record_scaling_list((scalingFactor_t *)dst, &sl);
        dst = tbl->scaling_list + 16 * 1360;
	hal_record_scaling_list((scalingFactor_t *)dst, &sl);
        dst = tbl->scaling_list + 80 * 1360;
	hal_record_scaling_list((scalingFactor_t *)dst, &sl);
	//memcpy((void*)&run->scaling_matrix_cache, scaling,
	//       sizeof(struct v4l2_ctrl_hevc_scaling_matrix));
}

static struct vb2_buffer *
get_ref_buf(struct rkvdec_ctx *ctx, struct rkvdec_h265_run *run,
			     ...
			     struct rkvdec_h265_run *run)
{
	...
	u32 min_cb_log2_size_y, ctb_log2_size_y, min_cb_size_y, ctb_size_y;
	u32 pic_width_in_min_cbs_y, pic_height_in_min_cbs_y;
	u32 stride_y, stride_uv;
	u32 width, height;
	u32 offset;
	dma_addr_t dst_addr;
	u32 reg, i;

	reg = RKVDEC_MODE(RKVDEC_MODE_H265);
	...
	stride_y = ((round_up(width, ctb_size_y)
		    * (sps->bit_depth_luma_minus8 + 8)) >> 3);
	stride_uv = ((round_up(width, ctb_size_y)
	             * (sps->bit_depth_chroma_minus8 + 8)) >> 3);

	dev_info(rkvdec->dev, "rkvdec_h265_run: fmt.width=%d fmt.height=%d width=%u height=%u ctb_size_y=%u stride_y=%u stride_uv=%u\n", dst_fmt->width, dst_fmt->height, width, height, ctb_size_y, stride_y, stride_uv);

	stride_y = round_up(stride_y, 256) | 256;
	stride_uv = round_up(stride_uv, 256) | 256;
	//stride_y = round_up(stride_y, 64);
	//stride_uv = round_up(stride_uv, 64);

	hor_virstride = stride_y;
	//ver_virstride = round_up(height, 8);
	ver_virstride = round_up(height, 64);
	y_virstride = hor_virstride * ver_virstride;

	if (sps->chroma_format_idc == 1)
		uv_virstride = (stride_uv * ver_virstride) / 2;
	else if (sps->chroma_format_idc == 2)
		uv_virstride = stride_uv * ver_virstride;
	else if (sps->chroma_format_idc == 3)
		uv_virstride = 2 * stride_uv * ver_virstride;
	else
		uv_virstride = 0;

	yuv_virstride = y_virstride + uv_virstride;

	dev_info(rkvdec->dev, "rkvdec_h265_run: stride_y=%u stride_uv=%u hor_virstride=%u ver_virstride=%u y_virstride=%u uv_virstride=%u yuv_virstride=%u\n", stride_y, stride_uv, hor_virstride, ver_virstride, y_virstride, uv_virstride, yuv_virstride);

	reg = RKVDEC_Y_HOR_VIRSTRIDE(stride_y / 16) |
	      RKVDEC_UV_HOR_VIRSTRIDE(stride_uv / 16) |
	...
	/* config cabac table */
	offset = offsetof(struct rkvdec_h265_priv_tbl, cabac_table);
	...
	// TODO: valid refs flag

	reg = RKVDEC_CUR_POC(sl_params->slice_pic_order_cnt);
	writel_relaxed(reg, rkvdec->regs + RKVDEC_REG_CUR_POC0);

	/* config hw pps address */
	offset = offsetof(struct rkvdec_h265_priv_tbl, param_set);
	writel_relaxed(priv_start_addr + offset,
		       rkvdec->regs + RKVDEC_REG_PPS_BASE);

	/* config hw rps address */
	offset = offsetof(struct rkvdec_h265_priv_tbl, rps);
	writel_relaxed(priv_start_addr + offset,
...
static int rkvdec_h265_adjust_fmt(struct rkvdec_ctx *ctx,
...
static int rkvdec_h265_start(struct rkvdec_ctx *ctx)
{
	struct rkvdec_dev *rkvdec = ctx->dev;
	struct rkvdec_h265_priv_tbl *priv_tbl;
	struct rkvdec_h265_ctx *h265_ctx;
	int ret;

	h265_ctx = kzalloc(sizeof(*h265_ctx), GFP_KERNEL);
	if (!h265_ctx)
		return -ENOMEM;

	priv_tbl = dma_alloc_coherent(rkvdec->dev, sizeof(*priv_tbl),
				      &h265_ctx->priv_tbl.dma, GFP_KERNEL);
	if (!priv_tbl) {
		ret = -ENOMEM;
		goto err_free_ctx;
	}

	h265_ctx->priv_tbl.size = sizeof(*priv_tbl);
	h265_ctx->priv_tbl.cpu = priv_tbl;
	memset(priv_tbl, 0, sizeof(*priv_tbl));
	memcpy(priv_tbl->cabac_table, rkvdec_h265_cabac_table,
	       sizeof(rkvdec_h265_cabac_table));
...
static void rkvdec_h265_stop(struct rkvdec_ctx *ctx)
{
	struct rkvdec_h265_ctx *h265_ctx = ctx->priv;
	struct rkvdec_dev *rkvdec = ctx->dev;

	dma_free_coherent(rkvdec->dev, h265_ctx->priv_tbl.size,
			  h265_ctx->priv_tbl.cpu, h265_ctx->priv_tbl.dma);
	kfree(h265_ctx);
}

static void rkvdec_h265_run_preamble(struct rkvdec_ctx *ctx,
				     struct rkvdec_h265_run *run)
{
	struct rkvdec_dev *rkvdec = ctx->dev;
	...
	// TODO: how many slices_params controls ??
	run->num_slices = 1;
	dev_info(rkvdec->dev, "rkvdec_h265_run_preamble: num_slices=%d elems=%d elem_size=%d nr_of_dims=%d\n", run->num_slices, ctrl ? ctrl->elems : -1, ctrl ? ctrl->elem_size : -1, ctrl ? ctrl->nr_of_dims : -1);
	ctrl = v4l2_ctrl_find(&ctx->ctrl_hdl,
...
static int rkvdec_h265_run(struct rkvdec_ctx *ctx)
{
	struct rkvdec_dev *rkvdec = ctx->dev;
	struct rkvdec_h265_run run;
	int i;
	...
	rkvdec_h265_run_preamble(ctx, &run);
	...
	writel(0x13, rkvdec->regs + RKVDEC_REG_PREF_LUMA_CACHE_ENABLE);
	writel(0x13, rkvdec->regs + RKVDEC_REG_PREF_CHR_CACHE_ENABLE);
	writel(1, rkvdec->regs + RKVDEC_REG_PREF_LUMA_CACHE_COMMAND);
	writel(1, rkvdec->regs + RKVDEC_REG_PREF_CHR_CACHE_COMMAND);

	/* Start decoding! */
	val = (run.pps->flags & V4L2_HEVC_PPS_FLAG_TILES_ENABLED) ? 0 : RKVDEC_WR_DDR_ALIGN_E;
	writel(RKVDEC_INTERRUPT_DEC_E | RKVDEC_TIMEOUT_E | val,
	       rkvdec->regs + RKVDEC_REG_INTERRUPT);

	return 0;
}

const struct rkvdec_coded_fmt_ops rkvdec_h265_fmt_ops = {
	.adjust_fmt = rkvdec_h265_adjust_fmt,
	.start = rkvdec_h265_start,
	.stop = rkvdec_h265_stop,
	.run = rkvdec_h265_run,
};
