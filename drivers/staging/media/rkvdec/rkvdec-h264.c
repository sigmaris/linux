			    ...
			    struct rkvdec_h264_run *run)
{
	...
	for (j = 0; j < RKVDEC_NUM_REFLIST; j++) {
		enum v4l2_h264_dpb_reference a_parity =
			(dec_params->flags & V4L2_H264_DECODE_PARAM_FLAG_BOTTOM_FIELD)
			? V4L2_H264_DPB_BOTTOM_REF : V4L2_H264_DPB_TOP_REF;
		enum v4l2_h264_dpb_reference b_parity =
			(dec_params->flags & V4L2_H264_DECODE_PARAM_FLAG_BOTTOM_FIELD)
			? V4L2_H264_DPB_TOP_REF : V4L2_H264_DPB_BOTTOM_REF;
			...
			for (; a < h264_ctx->reflists.num_valid; a++) {
				...
				if ((dpb[idx].reference & a_parity) == a_parity &&
					...
					set_ps_field(hw_rps, BOTTOM_FLAG(i, j),
						a_parity == V4L2_H264_DPB_BOTTOM_REF);
					i++;
					a++;
					break;
				}
			...
			for (; b < h264_ctx->reflists.num_valid; b++) {
				...
				if ((dpb[idx].reference & b_parity) == b_parity &&
					...
					set_ps_field(hw_rps, DPB_INFO(i, j),
					             idx | (1 << 4));
					set_ps_field(hw_rps, BOTTOM_FLAG(i, j),
						b_parity == V4L2_H264_DPB_BOTTOM_REF);
					i++;
					b++;
					break;
				}
